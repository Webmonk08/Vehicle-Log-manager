...
export function VehiclesListScreen({ navigation }: Props) {
  ...
  const [formTarget, setFormTarget] = React.useState<"new" | Vehicle | null>(null);
  ...
  const handleSubmit = async (values: Record<string, any>) => {
    ...
    if (formTarget && formTarget !== "new") {
      await vehiclesApi.update(formTarget.id, payload as any);
    } else {
      await vehiclesApi.create(payload as any);
    }
    setFormTarget(null);
    refresh();
  };
  ...
  const handleDelete = (vehicle: Vehicle) => {
    Alert.alert("Delete vehicle?", `"${vehicle.number}" will be removed.`, [
  ...
  return (
    <View style={styles.container}>
      ...
      <CreateEntityModal
        visible={!!formTarget}
        title={formTarget && formTarget !== "new" ? "Edit Vehicle" : "New Vehicle"}
        submitLabel={formTarget && formTarget !== "new" ? "Save changes" : "Save"}
        initialValues={formTarget && formTarget !== "new" ? { number: formTarget.number, type: formTarget.type, capacity: formTarget.capacity } : undefined}
        fields={[
