...
async def update_driver(client: AsyncClient, driver_id: str | uuid.UUID, **kwargs) -> dict:
    ...
    tbl = client.table("drivers")
    updated = await tbl.update(data).eq("id", str(driver_id)).execute()
    fetched = await tbl.select("*").eq("id", str(driver_id)).execute()
...
async def get_trip(client: AsyncClient, trip_id: str | uuid.UUID) -> dict | None:
    print(f"Getting trip {trip_id}")
    response = await client.table("trips").select(TRIP_SELECT).eq("id", str(trip_id)).execute()
    print(f"Trip {response.data[0]}")
    return response.data[0] if response.data else None

async def create_trip(client: AsyncClient, **kwargs) -> dict:
    data = {k: to_str(v) for k, v in kwargs.items() if v is not None}
    insert_response = await client.table("trips").insert(data).execute()
    new_id = insert_response.data[0]["id"]
    fetched = await client.table("trips").select(TRIP_SELECT).eq("id", new_id).execute()
    return fetched.data[0]

async def update_trip(client: AsyncClient, trip_id: str | uuid.UUID, **kwargs) -> dict:
    data = {k: to_str(v) for k, v in kwargs.items() if v is not None}
    tbl = client.table("trips")
    await tbl.update(data).eq("id", str(trip_id)).execute()
